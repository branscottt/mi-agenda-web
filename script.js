...
function renderNotificaciones(lista) {
  ...
  let html = '';
  lista.forEach(c => {
    const nombre = c.contacto?.nombre || c.nombreCliente || 'Cliente';
    const telefono = c.contacto?.telefono || c.telefonoCliente || '';
    const email = c.contacto?.email || '';
    const servicio = c.nombre || c.servicioNombre || 'Servicio';
    const fecha = c.fecha || '—';
    const hora = c.hora || '—';

    const tipoTexto = c.tipo === 'nueva' ? '🆕 Nueva reserva' : '⏰ Próxima cita (24h)';
    const claseTipo = c.tipo === 'nueva' ? 'new-reservation' : 'upcoming';

    // Preparar mensajes predefinidos
    ...
    const mensajeWhatsApp = encodeURIComponent(
      `Hola ${nombre}, recordatorio: tienes una cita de ${servicio} el ${fecha} a las ${hora}.`
    );
    const waLink = `https://wa.me/${telefono.replace(/\D/g, '')}?text=${mensajeWhatsApp}`;

    html += `
      <div class="notification-item ${claseTipo}">
        <div class="notification-info">
          <strong>${tipoTexto}</strong>
          <span>${nombre} - ${servicio} - ${fecha} ${hora}</span>
        </div>
        <div class="notification-actions">
          ${email ? `<a href="${mailtoLink}" target="_blank" class="btn-notify email"><i class="fas fa-envelope"></i> Email</a>` : ''}
          ${telefono ? `<a href="${waLink}" target="_blank" class="btn-notify whatsapp"><i class="fab fa-whatsapp"></i> WhatsApp</a>` : ''}
        </div>
      </div>
    `;
  });
...
function _renderCitasBase(contenedorId, opciones = {}) {
    ...
    const { soloUsuario = false, mostrarWhatsApp = false, mostrarFinalizar = false, mostrarCancelar = false, mostrarEditado = false } = opciones;
    const session = getSession();
    ...
    citas.forEach(c => {
        ...
        const hora = c.hora || '—';
        const editado = (mostrarEditado && c.editado) ? ' <span style="color:#ff9800;">(Editado)</span>' : '';
        ...
        if (mostrarFinalizar) {
            html += `<button class="btn-small btn-complete" data-id="${c.id}">Finalizar</button> `;
        }
        if (mostrarCancelar) {
            html += `<button class="btn-small btn-cancel-res" data-id="${c.id}">Cancelar</button>`;
        }
        html += `</td></tr>`;
    });
    ...
    container.querySelectorAll('.btn-complete').forEach(btn => {
        btn.addEventListener('click', function () {
            ...
            if (CitasManager.finalizar(id)) {
                _renderCitasBase(contenedorId, opciones);
...
function renderAdminAppointments() {
    _renderCitasBase('upcoming-appointments', { mostrarWhatsApp: true, mostrarFinalizar: true, mostrarEditado: true });
}
...
function aplicarFiltrosCombinados() {
    ...
    if (serviciosFiltrados.length === 0 && serviciosActivos.length > 0) {
        ...
        if (currentFilterTerm && currentFilterDate) {
            const fechaFormateada = new Date(currentFilterDate).toLocaleDateString('es-ES');
            mensaje = `No hay servicios con "${currentFilterTerm}" disponibles para el ${fechaFormateada}`;
